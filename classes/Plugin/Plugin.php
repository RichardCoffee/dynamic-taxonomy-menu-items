...
abstract class DynTaxMI_Plugin_Plugin {
...
#	 * @since 20170111
	 ...
	 *
	 * @since 20170113
	 * @var object  DynTaxMI_Plugin_Paths object.
	 */
	public $paths = null;
	/**
	 *
	...
	protected function __construct( $args = array() ) {
		if ( ! empty( $args['file'] ) ) {
			...
			$this->paths = DynTaxMI_Plugin_Paths::get_instance( $args );
	 ...
	 */
	public function get_stylesheet( $file = 'css/dyntaxmi.css' ) {
	...
	public function settings_link( $links, $file, $data, $context ) {
		if ( strpos( $file, basename( $this->paths->file ) ) ) {
			...
			if ( is_plugin_active( $file ) ) {
				...
				if ( $url ) $links['settings'] = sprintf( '<a href="%s"> %s </a>', esc_url( $url ), esc_html__( 'Settings', 'dyntaxmi' ) );
			}
		}
		return $links;
	}
